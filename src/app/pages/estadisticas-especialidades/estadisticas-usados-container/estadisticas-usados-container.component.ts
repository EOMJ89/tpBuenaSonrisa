...
@Component({
  selector: 'app-estadisticas-usados-container',
  templateUrl: './estadisticas-usados-container.component.html',
  styleUrls: ['./estadisticas-usados-container.component.css']
})
export class EstadisticasUsadosContainerComponent implements OnInit, OnDestroy {
  ...
  private manageList() {
    ...
    if (this._especialistas !== undefined && this._turnos !== undefined) {
      let lista = new Array<{ especialista: string, especialidad: string, turnos: number }>();
      ...
      lista = lista.sort(this.ordenarPorTurnos);
      this.estadisticas = new Array<{ especialista: string, especialidad: string, turnos: number }>(lista[0], lista[lista.length - 1]);
      // console.log(this.estadisticas);
    }
  }

  private ordenarPorTurnos(
    a: { especialista: string, especialidad: string, turnos: number },
    b: { especialista: string, especialidad: string, turnos: number }) {
    if (a.turnos > b.turnos) { return 1; }
    if (a.turnos < b.turnos) { return -1; }
    return 0;
  }
